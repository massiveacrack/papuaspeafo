...
def phases_from_signal(sig):
    cos_part = sig; sin_part = np.roll(sig, 1)  # proste ~90° przesunięcie
...
def compute_metrics(X, fs, win_sec=0.4, step_sec=None):
    ...
    SE_list=C_list=R_list=S_list=[]; SE_list=[]; C_list=[]; R_list=[]; S_list=[]; centers=[]
...
def main():
    ...
    ap.add_argument("--in",dest="inp",required=True); ap.add_argument("--fs",type=float,required=True)
    ap.add_argument("--win",type=float,default=0.4); ap.add_argument("--step",type=float,default=None)
    ap.add_argument("--out",type=str,required=True); ap.add_argument("--detrend",action="store_true")
    ap.add_argument("--alphaC",type=float,default=1.0); ap.add_argument("--alphaR",type=float,default=0.9)
    ap.add_argument("--alphaS",type=float,default=0.8); ap.add_argument("--beta",type=float,default=0.7)
    ap.add_argument("--theta_star",type=float,default=0.35); ap.add_argument("--T_star",type=int,default=30)
    ap.add_argument("--death_index",type=int,default=None)
    a=ap.parse_args(); os.makedirs(a.out,exist_ok=True)
    X=load_csv(a.inp); X=detrend_per_channel(X) if a.detrend else X
    m=compute_metrics(X,a.fs,a.win,a.step); SE,C,R,S,t=m["SE"],m["C"],m["R"],m["S"],m["t"]
    Theta=theta_from(SE,C,R,S,a.alphaC,a.alphaR,a.alphaS,a.beta)

    import csv; csvp=os.path.join(a.out,"sei_theta_timeseries.csv")
    with open(csvp,"w",newline="") as f:
        w=csv.writer(f); w.writerow(["center_index","SE","C","R","S","Theta"])
        for i in range(len(Theta)): w.writerow([int(t[i]),float(SE[i]),float(C[i]),float(R[i]),float(S[i]),float(Theta[i])])
    ...
    save_plot(C,"SEI: C","C","plot_C.png"); save_plot(R,"SEI: R","R","plot_R.png")
    save_plot(S,"SEI: S","S","plot_S.png"); save_plot(Theta,"SEI: Θ(t)","Theta","plot_Theta.png")

    persist=None
    if a.death_index is not None:
        idx=int(np.argmax(t>=a.death_index)); end=min(idx+a.T_star,len(Theta)); persist=bool(np.all(Theta[idx:end]>=a.theta_star))
